-- Fix leaderboard:
-- 1. today_points now correctly counts only finished matches from today (UTC)
-- 2. Tiebreaker last_correct_at changed from ASC to DESC (most recent wins tiebreak)
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT
        ...
        -- Fixed: only count finished matches from today (UTC), not future ones
        SUM(CASE
    ...
    ORDER BY
      ...
      -- Fixed: DESC so most recent correct prediction wins tiebreak (not oldest)
